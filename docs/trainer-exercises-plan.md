...
| S10 | **Landed in PR 3** | `supabase/functions/translate-name/index.ts` proxies Google Cloud Translation v2 with the API key in a Supabase Function secret (`GOOGLE_TRANSLATE_API_KEY`). Client `useTranslate()` hook calls `supabase.functions.invoke('translate-name', ...)`; no key in the browser bundle. Returns `null` + an error message on failure so the UI can degrade gracefully (the button is disabled while the other field is empty, and the user can always type the name manually). |
