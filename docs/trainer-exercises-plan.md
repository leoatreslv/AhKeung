...
| S6 | **Server in PR 1, client integration in PR 5** | RPC + idempotent emission landed in PR 1. PR 5 wires the client: `sharePlan(planId, recipient)` in `src/sharing.ts` invokes the RPC via `supabase.functions.rpc('share_plan', ...)` and surfaces success/failure inline in the PlanEditor footer. |
| S7 | **Server in PR 1, client integration in PR 5** | `superseded_by` column landed in PR 1. PR 5 surfaces it: the "Assigned by your trainer" card on Home filters to `assignedBy IS NOT NULL AND supersededBy IS NULL`, so the trainee sees only the current copy. Older copies still exist in the plans list. |
| S9 | **Server in PR 1, client integration in PR 5** | `trainer_trainees.status` + `has_accepted_designation()` SECURITY DEFINER landed in PR 1. PR 5 wires the trainee response flow: `DesignationBanner` on Home shows pending designations with Accept / Decline buttons that update `status` via the Supabase client (RLS lets the trainee update their own row). `Settings.YourTrainersSection` lists accepted trainers with a "Block" button that flips status → `declined`. ShareSheet sources its recipient list from accepted designations only. |
...
| O17 | **Partially landed post-PR 6** | Migration `0004_trainer_promotion_and_cascade.sql` tightens `profiles_write` so a user can't flip their own `is_trainer` flag (the WITH CHECK requires the new `is_trainer` to equal the existing value). Adds a SECURITY DEFINER `promote_to_trainer(target)` RPC callable only by trainers, surfaced in MyTrainees as a Promote button on accepted designations. Bootstrap (first trainer) still requires a service-role `UPDATE` from the SQL editor; that's by design — there's no in-app path to create the seed trainer. |
