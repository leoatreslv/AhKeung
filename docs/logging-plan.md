...
| S5 | **Landed in PR C (diagnostics_reports)** | Migration `0007_diagnostics_reports.sql`: SELECT policy joins through `trainer_trainees` with `status = 'accepted'`. No `is_trainer()`. `audit_events` half scheduled with PR D. |
